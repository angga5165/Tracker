...
function loadExpenses() {
  try {
    ...
    if (stored) {
      ...
      // Migrate existing entries to have type, source, etc.
      let migrated = false;
...
function getDailyIncomeSummaries(year, month) {
  const expenses = loadExpenses();
  const daysInMonth = new Date(year, month, 0).getDate();
  const summaries = [];
...
function viewDashboard() {
  ...
  const monthIncome = getMonthIncomeTotal(currentYear, currentMonth);
...
function postRenderDashboard() {
  ...
  const dailyIncomeData = getDailyIncomeSummaries(currentYear, currentMonth);
...
function postRenderPengaturan() {
  ...
  saveBalanceBtn?.addEventListener('click', () => {
    ...
    // Trigger dashboard total reload in case the user navigates there
    setTimeout(() => {
...
function exportToExcel(year, month) {
  ...
  // saldo awal (dari Pengaturan) + semua transaksi SEBELUM bulan ini
  const startOfMonthStr = activeMonthStr + '-01';
  const balanceInitial = loadBalance();
  let startDompet = balanceInitial.initialDompet;
  let startATM = balanceInitial.initialATM;
